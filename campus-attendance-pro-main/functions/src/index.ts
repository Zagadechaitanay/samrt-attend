 ...
 */
export const autoMarkAbsentees = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (before.status !== "completed" && after.status === "completed") {
      try {
        ...
        console.log(
          `Total students: ${studentsSnapshot.size}, Present: ${presentStudentIds.size}`
        );
        ...
        if (absentCount > 0) {
          ...
          for (const studentId of absentStudentIds) {
            notifyParentOfAbsence(studentId, sessionId, after).catch((error) => {
              console.error(
                `Error notifying parent for student ${studentId}:`,
                error
              );
            });
          }
 ...
 */
export const calculateAttendancePercentage = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      console.log(
        `Calculating attendance for student ${studentId}, subject ${subjectId}`
      );
      ...
      console.log(
        `Updated attendance: Student ${studentId}, Subject ${subjectId}: ${roundedPercentage}% (Overall: ${roundedOverallPercentage}%)`
      );
      ...
      if (roundedPercentage < 75 && totalClasses >= 5) {
        ...
        checkAndNotifyLowAttendance(studentId, subjectId, roundedPercentage).catch(
          (error) => {
...
async function notifyParentOfAbsence(
  ...
  try {
    ...
    const subjectName = subjectDoc.data()?.name || "Unknown Subject";
    ...
    console.log(
      `Created notification ${notificationRef.id} for parent ${parentId}`
    );
...
async function checkAndNotifyLowAttendance(
  ...
  try {
    ...
    if (!recentNotification.empty) {
      console.log(
        `Low attendance notification already sent recently for student ${studentId}`
      );
      return;
    }
    ...
    const notificationData = {
      ...
      message: `${studentData?.name || "Your child"}'s attendance in ${subjectName} is ${percentage.toFixed(1)}%. Please ensure regular attendance.`,
      student_id: studentId,
      subject_id: subjectId,
      read: false,
      fcm_sent: false,
    ...
    console.log(
      `Low attendance notification sent for student ${studentId}, subject ${subjectId}`
    );
  } catch (error) {
    console.error("Error checking low attendance:", error);
  }
 ...
 */
export const recalculateAttendance = functions.https.onRequest(
  async (req, res) => {
    // Enable CORS
